/* Service worker nhận thông báo khi app đóng hoặc máy đang khoá.
   Bắt buộc dùng bản compat — service worker không hỗ trợ ES module import. */
...
// Chỉ chạy với payload dạng data-only. Nếu máy chủ gửi kèm khối "notification",
// trình duyệt tự hiện thông báo và hàm này không được gọi.
...
self.addEventListener("notificationclick", event => {
  ...
  event.waitUntil(
    clients.matchAll({ type: "window", includeUncontrolled: true }).then(list => {
      for (const c of list) {
        if (c.url.includes("/apps/") && "focus" in c) return c.focus();
      }
